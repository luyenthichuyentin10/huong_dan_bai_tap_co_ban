## 🗑️ Bài 6: Các thao tác Xóa phần tử trên mảng
...
* **Xóa tất cả giá trị $X$:** 
    ...
    * *Cách 2 (Tối ưu tốc độ, tốn bộ nhớ gấp đôi):* Tạo mảng mới chỉ chứa các phần tử khác $X$.
* **Xóa tất cả giá trị $X$ đến cuối:** Ta chỉ cần thay đổi biến quản lý số phần tử của mảng `na = x`
* **Xóa tất cả giá trị $X$ đến đầu:** 
    * *Cách 1:* Duyệt từ vị trí `i = 0` đến `x`, mỗi bước duyệt gọi hàm `XoaTaiViTri(A, n, 0)` để xóa và dồn mảng.
    * *Cách 2:* Duyệt từ vị trí `i = x + 1` đến cuối, mỗi bước duyệt gán `A[i-(x+1)] = A[i]`, giảm biến quản lý số phần tử của mảng `n = n - x`
    * *Cách 3 (Tối ưu tốc độ, tốn bộ nhớ gấp đôi):* Tạo mảng mởi, lưu các phần tử trong mảng `A` từ vị trí `x + 1` đến cuối qua mảng mới
* **Xóa tổng quát từ vị trí $X$ xóa đi $Y$ phần tử về cuối:** 
    * *Cách 1:* Duyệt từ vị trí `i = x` đến `i <= y + 1`, mỗi bước duyệt gọi hàm `XoaTaiViTri(A, n, x)` để xóa và dồn mảng.
    * *Cách 2: (Tối ưu tốc độ, tốn bộ nhớ gấp đôi):* Tạo mảng mởi, lưu các phần tử trong mảng `A` qua mảng mới ngoại trừ các vị trí từ `x` đến `x + y`
