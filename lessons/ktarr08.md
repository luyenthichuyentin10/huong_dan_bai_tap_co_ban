...
Công ty khai thác trái cây quý thu được $N$ viên trái cây quý. Mỗi viên có chỉ số độ ngọt $A_i$. Một viên đá được gọi là **"trái cây quý"** khi thỏa mãn **đồng thời hai điều kiện**:
* Chỉ số độ ngọt là một **số nguyên tố**.
* Chỉ số đó là **duy nhất** trong toàn bộ vụ thu hoạch (không trùng với viên nào khác).

**Yêu cầu:**
1. Sắp xếp các viên trái cây quý (đủ điều kiện) theo chỉ số **giảm dần**.
...
- Dòng đầu: danh sách trái cây quý đã sắp xếp giảm dần.
- $Q$ dòng tiếp: mỗi dòng ghi tổng chỉ số từ $L$ đến $R$ (nếu $R$ vượt số lượng trái cây quý thì tính đến viên cuối).
...
Trái cây quý (giảm dần): `19 17 11 7 5`.
...
**Bước 3 — Lọc trái cây quý:** `5, 7, 11, 17, 19` (nguyên tố + duy nhất).
...
<span class="kw">HÀM</span> <span class="fn">DaQuy</span>(<span class="var">a</span>[], <span class="var">n</span>, <span class="var">queries</span>[]):
    ...
    <span class="com">// Bước 2: Lọc trái cây quý (nguyên tố + xuất hiện đúng 1 lần)</span>
